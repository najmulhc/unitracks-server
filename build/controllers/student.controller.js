...
const studentInputPhaseOne = async (req, res) => {
    ...
    if (student?.authStage !== "one") {
        throw new ApiError_util_1.default(401, "you are in wrong auth phase!");
    }
    ...
    if (!firstName || !lastName || !dateOfBirth || !bloodGroup) {
        throw new ApiError_util_1.default(400, "Incomplete user information, please give full info");
    }
    const bloodGroups = ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"];
    if (!bloodGroups.includes(bloodGroup)) {
        throw new ApiError_util_1.default(400, "Invalid blood group.");
    }
    const realDateOfBirth = new Date(dateOfBirth).getTime();
    if (!realDateOfBirth || typeof realDateOfBirth !== "number") {
        throw new ApiError_util_1.default(400, "Invalid date of birth.");
    }
...
const studnetInputPhaseTwo = async (req, res) => {
    ...
    if (!roll || !session) {
        throw new ApiError_util_1.default(400, "Incomplete form informaiton, we need full information");
    }
    const studentWithRoll = await student_model_1.default.findOne({
        roll,
    });
    if (studentWithRoll) {
        throw new ApiError_util_1.default(401, "Student already exists with this roll.");
    }
    const sessions = ["2020", "2019"];
    if (!sessions.includes(session)) {
        throw new ApiError_util_1.default(400, "Invalid session! please provide the correct one.");
    }
