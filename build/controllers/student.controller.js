"use strict";
// input phase 1 (basic personal information)
// input phase 2 (academics information)
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.studnetInputPhaseTwo = exports.studentInputPhaseOne = exports.getStudent = void 0;
var student_model_1 = __importDefault(require("../models/student.model"));
var ApiError_1 = __importDefault(require("../utils/ApiError"));
var getStudent = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, role, student;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, email = _a.email, role = _a.role;
                // varify user is student
                if (role !== "student") {
                    throw new ApiError_1.default(400, "You are not a student");
                }
                return [4 /*yield*/, student_model_1.default.findOne({
                        email: email,
                    })];
            case 1:
                student = _b.sent();
                if (!student) {
                    throw new ApiError_1.default(404, "Student not found.");
                }
                return [2 /*return*/, res.json({
                        success: true,
                        student: student,
                    })];
        }
    });
}); };
exports.getStudent = getStudent;
// student input phase one
var studentInputPhaseOne = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, role, student, _b, firstName, lastName, dateOfBirh, bloodGroup, bloodGroups, realDateOfBirth, updatedStudent;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _a = req.body, email = _a.email, role = _a.role;
                // varify user is student
                if (role !== "student") {
                    throw new ApiError_1.default(400, "You are not a student");
                }
                return [4 /*yield*/, student_model_1.default.findOne({
                        email: email,
                    })];
            case 1:
                student = _c.sent();
                if (!student) {
                    throw new ApiError_1.default(404, "Student not found.");
                }
                if (student.authStage !== "one") {
                    throw new ApiError_1.default(400, "you are in wrong auth phase!");
                }
                _b = req.body, firstName = _b.firstName, lastName = _b.lastName, dateOfBirh = _b.dateOfBirh, bloodGroup = _b.bloodGroup;
                // varify the information given as per request.
                if (!firstName || !lastName || !dateOfBirh || !bloodGroup) {
                    throw new ApiError_1.default(400, "Incomplete user information, please give full info");
                }
                bloodGroups = ["A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"];
                if (!bloodGroups.includes(bloodGroup)) {
                    throw new ApiError_1.default(400, "Invalid blood group.");
                }
                realDateOfBirth = new Date(dateOfBirh).getTime();
                if (!realDateOfBirth || typeof realDateOfBirth !== "number") {
                    throw new ApiError_1.default(400, "Invalid date of birth.");
                }
                return [4 /*yield*/, student_model_1.default.findOneAndUpdate({ email: email }, {
                        firstName: firstName,
                        lastName: lastName,
                        dateOfBirth: realDateOfBirth,
                        bloodGroup: bloodGroup,
                    }, { new: true })];
            case 2:
                updatedStudent = _c.sent();
                // return new student object
                return [2 /*return*/, res.json({
                        success: true,
                        student: updatedStudent,
                    })];
        }
    });
}); };
exports.studentInputPhaseOne = studentInputPhaseOne;
var studnetInputPhaseTwo = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        return [2 /*return*/];
    });
}); };
exports.studnetInputPhaseTwo = studnetInputPhaseTwo;
