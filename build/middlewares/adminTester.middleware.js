...
const adminTester = (0, asyncHandler_util_1.default)(async (req, res, next) => {
    try {
        ...
        const foundAdmin = await admin_model_1.default.findOne({
            email
        });
        if (!foundAdmin) {
            throw new ApiError_util_1.default(404, "Admin is not found.");
        }
        req.admin = foundAdmin;
        next();
    }
    catch (error) {
        throw new ApiError_util_1.default(400, error.message || "There was an error testing admin previlages.");
    }
});
